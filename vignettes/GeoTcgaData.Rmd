...
# Use `differential_RNA` to do difference analysis. We provide the data 
...
# use `methydifferential_ucsc` to do difference analysis   
...
# variation data using `differential_CNV`
...
1. The function `id_conversion` could convert gene id from one 
...
```{r}
id_conversion("symbol", "ensembl_gene_id", c("A2ML1", "A2ML1-AS1", 
    "A4GALT", "A12M1", "AAAS")) 
```
...
`id_conversion` used HGNC data to convert human gene id.  
